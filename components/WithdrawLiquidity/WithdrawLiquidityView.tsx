...
const WithdrawLiquidity = ({sliderPercentage, changeSliderPercentage, sliderPercentChange, withdrawLiquidity, approveTransaction, token0, token1, liquidityAmt, liqInTokB, enableRemove}: WithdrawLiquidityProps) => {
    ...
    const style = {
        ...
        successButton: "w-full bg-green-400 m-2 rounded-2xl py-4 px-6 text-xl font-semibold flex justify-center items-center cursor-pointer text-white mt-8 border-2 border-green-400 hover:border-green-300",
      ...
      const withdraw = (amount: number) => {
        withdrawLiquidity(amount).then(res => {
          console.log(res)
        }).catch(err => {
          console.log(err)
        })
            ...
            <div className={style.wrapper}>
              <div className={style.content}>
                ...
                <div>
                  ...
                  <div className={style.totalPriceDiv}>
                    ...
                    <div className={style.totalPrice}>
                      <p>1 {token0.symbol || '-'} = 4295.45 {token1.symbol || '-'}</p>
                      <p>1 {token0.symbol || '-'} = 0.000232804 {token1.symbol || '-'}</p>
                    </div>
                  </div>
                ...
                <div className={style.buttonDiv}>
                    ...
                    {enableRemove ?
                    <div className={style.successButton} onClick={() => {withdraw(sliderPercentage[0])}}>
                        Remove
                    </div>
                    : 
